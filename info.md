...
- **Non-Essential entity level** — mark entities as non-essential: shown on the card but excluded from all KPIs (offline count, availability %, MTBF, MTTR) and alerts. Useful for expected-offline devices (TV in standby, printer off between jobs) without needing a separate group. Set in the group creation step alongside entity selection.
