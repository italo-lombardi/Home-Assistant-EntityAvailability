...
|--------|------|-------|-------|
| `sensor..._combined_summary` | Sensor | Total offline count across all source groups | Attributes: `total_entities`, `online`, `offline`, `stale`, `low_battery`, `suppressed`, `non_essential`, `non_essential_entities`, `battery_powered`, `battery_enabled`, `staleness_enabled`, `entities`, `groups`, `offline_entities`, `low_battery_entities`. The `groups` attribute is a dict keyed by `entry_id`: `{entry_id: {name, entity_id, total, online, offline, stale, low_battery, suppressed, non_essential, non_essential_entities, non_essential_online, non_essential_offline, non_essential_stale, non_essential_low_battery, battery_enabled, staleness_enabled, battery_powered}}`. `total` excludes non-essential entities. `missing_groups` (list of entry IDs) is present when one or more source groups are not loaded. |
