...
- **`any_stale` binary sensor contradicted `stale_count` / `stale_entities`** — a device that was both offline **and** stale (its staleness age kept climbing while it sat `unavailable`, especially with "count attribute updates as activity" on) flipped `binary_sensor.*_any_stale` **ON** while `sensor.*_stale_count` stayed `0` and `stale_entities` read `"None"`. The binary iterated the raw device set with no offline filter; the count/list sensors iterate the collapsed representatives **and** exclude offline devices (an offline+stale device surfaces as OFFLINE, not STALE). The binary now routes through the same collapsed, offline-excluded predicate, so `any_stale` is ON **iff** `stale_count > 0`. Automations keyed on the binary no longer fire with no identifiable stale entity. (#105)
- **Open outages skewed MTBF / MTTR** — while a device was currently offline (outage not yet recovered), MTTR read too low and MTBF too high. The offline-event count increments when an outage **opens** but total offline time is only booked at **recovery**, so an open outage inflated the divisor with no matching downtime. Reliability stats now divide by the **completed**-outage count, subtract the in-progress outage's elapsed time from uptime (so ongoing downtime is never counted as uptime), and keep MTTR to completed repairs only. A device whose only outage is still open now reports `None` (matching the documented "None until a full offline→recovery cycle" contract) instead of a misleading `MTTR = 0.0`. (#105)
- **`availability_*` rolling-window sensors wrote a recorder row every poll** — the group availability-% sensors (`today`, `3d`, `5d`, `7d`) republished on almost every coordinator poll purely from rounding jitter, with no real availability change (~7000 rows/day for the battery `today`+`3d` sensors alone). The percentage is computed over 5-minute buckets; the current **in-progress** bucket accrued online time gradually while real (elapsed) time also grew, so its ratio wobbled every poll and the rounded whole-window % crossed a 0.1 boundary each poll. The ratio is now computed from **completed buckets only** — the in-progress bucket is excluded once at least one bucket has closed — so the value changes only when a bucket completes (a real 5-minute data point) and stays stable between polls. A brand-new entity with no completed bucket yet still shows a value immediately from its in-progress bucket (a one-time startup ramp, not steady-state jitter). Trade-off: the % lags reality by up to one bucket (≤5 min); `offline_count` and the binary sensors give instant status. The write-dedup comparison is unchanged (still on the rounded value). (#105, #106)

## [0.5.3] - 2026-09-14

### Fixed
- **Recorder write amplification on the combined summary sensor (~2200 writes/day)** — the combined `combined_summary` value oscillated between adjacent counts (e.g. 133↔134) all day with no real state change. The device-collapse key was built from **live sensor values** (`battery_level`, `signal_level` rounded to an int), so a signal reading naturally drifting across a boundary (e.g. −70↔−71 dBm) changed the key every poll, reshuffled which entities merged into one device row, and nudged the summary count by one — each nudge a recorder write. Collapse now keys on the **source** (which battery/signal *sensor* an entity uses — static configuration that never oscillates), not on the value. A drifting reading can no longer move a row. Multi-sensor devices are unaffected: two entities that use **different** battery sensors still render as two rows even when they share one signal sensor. (#99)
- **Threshold-boundary flag flapping on the recorded `low_battery` / `stale` / `poor_signal` counts** — a reading resting exactly on a threshold (battery at the configured %, staleness age at the minute, signal at the poor/ok dBm) flipped the derived flag every poll, oscillating the recorded count attribute and the status colour. Each flag now has a small de-jitter dead-band: it turns **on** at the threshold but only turns **off** once the reading clears the threshold by a margin (battery 3%, staleness 1 min, signal 3 units). A stable reading on the boundary stays put. Signal de-jitter covers the **poor/ok** boundary only — the sole signal boundary feeding a recorded count; the good/ok boundary is left as-is (a good↔ok wobble is display-only, not a recorder write). (#99)
- **A flat battery (`battery_level: 0`) read as a sibling's level** — an entity reporting a genuine 0% battery attribute was skipped (0 is falsy) and its battery silently sourced from a `device_class: battery` sibling or guessed sensor, so a dead battery could display as healthy. A 0% reading is now taken as the real level (own entity), matching the source-based collapse provenance. (#99)
- **Combined groups now respect each source group's own collapse setting** — combined severity lists (`offline_entities`, `low_battery`) previously collapsed a device's entities into one row whenever the owning group had **Show device names** on, ignoring that group's **Collapse entities by device** toggle; combined recovery lists (`recently_offline`, `recently_recovered`) went the other way and *always* deduped by device id regardless of either toggle. The two paths disagreed. Both now follow the same gate a standalone group uses — **collapse active = Show device names AND Collapse entities by device** — applied per source group. A device's entities merge in the combined view only when their owning group is collapse-active; otherwise they stay separate rows, exactly as they would standalone. When the same physical device appears through two *different* collapse-active groups it still counts once, and device-less entities always stay their own row. (This corrects the doubled name a real tenant saw on a collapse-**off** smart-devices group: the group is collapse-off, so both entities are kept — but the identical device-name strings now fold to `"<name> {N}"`, see below, instead of rendering `"Name, Name"`.) (#98, #99, #100)
- **Duplicate display strings in every list sensor now fold to `"<name> {N}"`** — when two rows resolve to the same display string (two entities on a device shown by device name, two device-less entities sharing a friendly name), the list rendered the name twice (`"Balcony, Balcony"`). All list sensors — single-group and combined, severity and recovery — now group identical display strings and render `"<name>"` for one row or `"<name> {N}"` for N rows. `N` is the post-collapse row count, so it agrees with the numeric count sensors; `{N}` is a render-time marker only and never part of a device or entity name. This keeps the recorded state string stable (no more per-tick flap between orderings of a doubled name) while still surfacing that more than one entity is behind the name. (#100)
...
### Changed
- **All severity list sensors are now name-sorted** (case-insensitive, entity-id tiebreak), matching the recovery lists — `offline_entities`, `low_battery`, `stale`, and `poor_signal` (single-group and combined) previously rendered in coordinator/dict-insertion order. Severity and recovery lists within a group now share one deterministic order, and a stable set no longer produces spurious recorder writes from drifting iteration order. (#100)
- **`DeviceState` now carries `battery_source` / `signal_source`** (which sensor supplied each reading). These are computed live each refresh and are **not** persisted (recomputed on load); they drive the source-based collapse above. Battery source honours the existing provenance chain (auto-detected `device_class: battery` entity → its own id; explicit map → the mapped sensor; otherwise none/wildcard).
