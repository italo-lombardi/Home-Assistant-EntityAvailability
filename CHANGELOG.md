...
- **Reliability (MTBF) sensor** — a new `Reliability (MTBF)` sensor per group that flags devices which keep flaking out. It shows how *often* devices break (MTBF, mean time between failures — the sensor state, in hours) and how *long* each outage lasts (MTTR, mean time to recovery). This distinguishes a genuinely flaky device from one that had a single long outage at the same uptime %. Attributes: `mttr_minutes`, `total_offline_events`, and a `per_device` breakdown (`mtbf_hours`, `mttr_minutes`, `offline_events`). Derived from all-time event counters (no extra storage growth, no `state_class` so it does not generate long-term statistics). Values populate once an entity has completed at least one offline→recovery cycle. Combined-group counterpart is not included in this release.
