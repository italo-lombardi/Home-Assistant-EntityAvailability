...
- **`availability_*` rolling-window sensors wrote a recorder row every poll** — the group availability-% sensors (`today`, `3d`, `5d`, `7d`) republished on almost every coordinator poll purely from rounding jitter, with no real availability change (~2000+ rows/day for the `today` sensors alone). The current in-progress 5-minute bucket claimed the full 300 s as its denominator the instant it was created while online time accrued gradually, so a steadily-online device's ratio drifted upward every poll and crossed a rounding boundary repeatedly. The in-progress bucket's denominator now uses **elapsed** time (computed at read time, so a restart mid-bucket can't resurface the drift), so a stable device reports a flat percentage and no longer republishes. The write-dedup comparison is unchanged (still on the rounded value). (#105)
