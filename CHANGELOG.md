...
### Added
- **Combined groups now fire `entity_availability_offline` and `entity_availability_recovered` events** — previously only individual groups fired these events, making it impossible to trigger automations directly on a combined group's aggregate state. Combined groups now fire the same events with payload fields `group`, `entry_id`, `offline_count`, and `offline_entities`. Note: combined events omit `entity_id` (no single triggering entity — use `offline_entities` instead). Both events can fire in the same update tick when one entity goes offline and another recovers simultaneously, matching individual group parity. Events fire only when the merged offline set changes; rapid coordinator updates from multiple member groups do not cause duplicate fires.
